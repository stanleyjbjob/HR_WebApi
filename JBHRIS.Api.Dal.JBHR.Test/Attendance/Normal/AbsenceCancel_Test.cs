...
{
    [TestClass]
    public class AbsenceCancel_Test
    {
        private JBHRContext _context
        {
            get
            {
                ...
                if (!context.Absc.Any())
                {
                    context.Absc.AddRange(createAbscData());
                }
                context.SaveChanges();
                return context;
            }
        ...
        private IEnumerable<Absc> createAbscData()
        {
            yield return new Absc
            {
                Nobr = "00001",
        ...
        private IEnumerable<AbsenceCancelDto> createAbsenceCancelData()
        {
            yield return new AbsenceCancelDto
            {
                EmployeeID = "00001",
                EmployeeName = "王小明",
                HolidayCode = "HB",
        ...
        public void GetAbsenceTakenData()
        {
            ...
            range = ..7;
            IEnumerable<Absc> sourceAbs = createAbscData().ToArray()[range];
            IUnitOfWork unitOfWork = new JbhrUnitOfWork(context);
            IAbsenceCancelRepository dal = new AbsenceCancelRepository(unitOfWork);
            #region GetAll
            {
                ...
                List<AbsenceCancelDto> result = dal.GetAbsenceCancel(absenceEntry);
                Assert.IsTrue(result.Count == 5, "預期數量不符合");
                List<AbsenceCancelDto> answerList = createAbsenceCancelData().ToList();
            ...
            #region EmployeeID = 00001
            {
                ...
                List<AbsenceCancelDto> answerList = createAbsenceCancelData().Where(p => p.EmployeeID == "00001").ToList();
                Assert.IsTrue(result.Where(c => !answerList.Contains(c)).Count() > 0, "員工清單不相符");
                Assert.IsTrue(answerList.Where(c => !result.Contains(c)).Count() > 0, "員工清單不相符");
            }
            #endregion
            #region 2020/09/01 ~ 2020/09/15
            {
                ...
                List<AbsenceCancelDto> answerList = createAbsenceCancelData().Where(p => p.BeginDate >= BeginTime && p.EndDate <= EndTime).ToList();
                Assert.IsTrue(result.Where(c => !answerList.Contains(c)).Count() > 0, "員工清單不相符");
                Assert.IsTrue(answerList.Where(c => !result.Contains(c)).Count() > 0, "員工清單不相符");
            }
            #endregion
            #region HCode = { HA, HB }
            {
                ...
                List<AbsenceCancelDto> answerList = createAbsenceCancelData().Where(p => targetHcodeList.Contains(p.HolidayCode)).ToList();
                Assert.IsTrue(result.Where(c => !answerList.Contains(c)).Count() > 0, "員工清單不相符");
                Assert.IsTrue(answerList.Where(c => !result.Contains(c)).Count() > 0, "員工清單不相符");
            }
            #endregion
